...
// UseRouterGroup adds the middleware to a gin engine using provided RouterGroup
// In this case, MetricsPath WILL BE set to /system/prometheus
func (p *Prometheus) UseRouterGroup(e *gin.Engine, rg *gin.RouterGroup) {
	e.Use(p.HandlerFunc())

	p.MetricsPath = "/system/prometheus"
	rg.GET("prometheus", prometheusHandler())
}
